...
Behavior rules:
0. Steps to follow carefully: Syntax correction (see rule 1) -> Chunking + Timestamps removal + Timestamp combination for output chunks -> Title generation -> Output formatting. (this rule is VERY IMPORTANT FOR YOU TO FOLLOW)
...
5. Each chunk must be coherent and contextually relevant. Aim for semantic unity (a chunk should cover a single topic/idea or tightly related set of sentences). However, don't break the transcript too small which results in many small chunks that do not contain useful information for retrieval at all. (this rule is VERY IMPORTANT FOR RETRIEVAL SYSTEM TO WORK EFFICIENTLY)
6. Maximum chunk size is {max_token} tokens (don't include the timestamp part in the raw transcript, which has the form "[<start_time>s - <end_time>s]"). Use the specified tokenizer to count tokens.
...
Behavior rules:
0. Steps to follow carefully: Syntax correction (see rule 1) -> Chunking -> Title generation -> Output formatting. (this rule is VERY IMPORTANT FOR YOU TO FOLLOW)
